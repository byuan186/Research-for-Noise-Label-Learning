...
常见做法：
$$
\mathcal{L}_{\text{Dist}} = \sum_{i=1}^{N} \left[ D_{\text{KL}}(f_L(\boldsymbol{x}_i) \| f(\boldsymbol{x}_i)) + D_{\text{KL}}(f_N(\boldsymbol{x}_i) \| f(\boldsymbol{x}_i)) \right]
$$
对于一个损失较大的困难样本，fL倾向于将其视为尾类样本，增加了对该样本的学习关注。相比之下，fN很可能将其视为噪声样本，并可能减少或放弃从该样本中学习。如方程( 1 )所示的直接加权多教师蒸馏方法不能解决这种内在冲突，并可能导致次优结果

方法提出：

核心思想：
    设fL和fN为辅助模型，分别使用长尾学习算法AL和噪声标记学习算法AN在数据集D上进行训练。
    类别不平衡是数据集层面的分布特性，而标签噪声则与个体样本有关。我们提出fL应该在分布水平上指导目标模型f，而fN应该在样本水平上提供指导。

代理标签 $Q$ 的作用：$Q$ 是一个 $C\times N$ 的矩阵（$C$ 为类别数，$N$ 为批次大小），其中每一列 $q_i$ 代表一个样本 $x_i$ 的软标签（概率分布）。Q = [ q1 , q2 , ... , qN]∈ RC × N
